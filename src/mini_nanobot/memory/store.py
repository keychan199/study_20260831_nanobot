...
class MemoryStore:
    ...
    # 其余：write / read_all / read_history / read_history_since /
    # get|set_dream_cursor / snapshot / restore / has_changes
    # 全部直接调对应 helper，不要再套 asyncio.to_thread。
    #
    # 同样挂上：
    # snapshot_content = snapshot
    # restore_content = restore
...
def _directory_lock(self, path: Path) -> threading.RLock:
...
summary：LLM 压缩成功，或中间件把 Summarization 结果归档。这是正常摘要。
raw：压缩时 LLM 失败，Consolidator 把原文截断存进去，避免流程卡住。
成功走 kind = "summary"，失败 kind = "raw"。
'''
...
def _validate_memory_name(name: str) -> None:
    '''
    工具只能改这三份。名字不在 MEMORY_FILES 里直接拒绝，防止写成 history.jsonl 或任意      路径。
    '''
...
def _initialize(directory: Path) -> None:
    '''
    记忆目录第一次用时建出来。目录不存在就创建；三份 Markdown 缺哪份补哪份默认内容。已有文件不动，避免把用户改过的记忆覆盖掉。加锁是防止两个后端同时初始化写出两份半成品
    '''
...
def _write_text(directory: Path, path: Path, content: str) -> None:
    '''
    真正改某一份文件。先拿目录锁，再 _atomic_write（写临时文件再 replace），避免写到一半崩溃留下残缺文件。
    '''
    with _directory_lock(directory):
...
def _snapshot(directory: Path) -> MemorySnapshot:
    '''
    把三份文件读成一个 dict[str, str]，相当于改之前的备份。Dream 压缩前留档
    '''
...
def _restore(directory: Path, snapshot: Mapping[str, str]) -> None:
    '''
    用快照把三份文件写回去。快照必须刚好是这三份、值必须是字符串，然后加锁逐个原子写。Dream 中途失败就走这里，半成品改动会被抹掉。
    '''
